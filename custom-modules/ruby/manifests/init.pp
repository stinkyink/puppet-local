...
# setup ruby2.0 packages and install common dependencies and gems,
# including bundler.
#
# If $system_default is true, then ruby2.0 will be the system-wide default
# version of ruby

class ruby::ruby2($system_default = true)  {
  include ruby::common
  package { ["ruby2.0", "ruby2.0-dev"]:
    ensure => installed
  }
  Package["ruby2.0"] -> Package["ruby"]
  Package["ruby2.0-dev"] -> Package["ruby-dev"]

  Package["ruby2.0"] -> Package["bundler"]
  Exec["ruby2.0-install-bundler"] -> Package["bundler"]

  exec { "ruby2.0-install-bundler":
    command => "/usr/bin/gem2.0 install -f --no-rdoc --no-ri --format-executable --bindir /usr/local/bin  bundler",
    creates => "/usr/local/bin/bundle2.0",
    require => Package["ruby2.0"]
  }

  exec { "bundle2.0-alternatives":
    command => "/usr/sbin/update-alternatives --install /usr/bin/bundle bundle /usr/local/bin/bundle2.0 2",
    require => Exec["ruby2.0-install-bundler"]
  }
